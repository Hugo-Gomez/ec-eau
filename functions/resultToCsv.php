...
function resultToCsv($conn){
    ...
    var_dump($res);
    fputcsv($file, array('mail','age','sexe','ville','cp','freqDouche','tempsDouche','debitDouche','freqBain','rempBain','freqDents','eauDents','freqVaisselle','choixVaisselle','methVaisselle','couvVaisselle','freqMal','dateMal','ecoeau','plante','momentPlante','eauPlante','freqPlante','voiture','methVoiture','freqVoiture','piscine','freqPiscine','volumePiscine','etiquette','bouteille'));
    foreach ($res as $row) {
        fputcsv($file, $row);
    }
    fclose($file);
